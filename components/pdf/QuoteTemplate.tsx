...
export default function QuoteTemplate(props: QuoteTemplateProps) {
  ...
  return (
    <html>
      <head>
        <meta charSet="utf-8" />
        <title>Devis {props.quoteNumber}</title>
        <style>{`
          * {
            margin: 0;
            padding: 0;
      ...
      <body>
        <div className="header">
          <div className="company-info">
            {showCompanyInfo && (
              <>
                ...
                {props.companyCountry && <p>{props.companyCountry}</p>}
                {props.companyPhone && <p>Tél: {props.companyPhone}</p>}
                {props.companyEmail && <p>Email: {props.companyEmail}</p>}
                {props.companyWebsite && <p>Web: {props.companyWebsite}</p>}
          ...
          <div className="quote-title">
            <h1>DEVIS</h1>
            <p>N° {props.quoteNumber}</p>
            <p>Date: {formatDate(props.quoteDate)}</p>
            <p>Valide jusqu'au: {formatDate(props.validUntil)}</p>
          </div>
        </div>

        <div className="customer-box">
          <div className="label">Devis pour:</div>
          <div className="content">
            ...
            {props.customerPhone && <div>{props.customerPhone}</div>}
          </div>
        </div>
        ...
        {props.notes && (
          <div className="notes-box">
            ...
            <div className="content">{props.notes}</div>
          </div>
        )}

        {showBankInfo && props.bankIBAN && (
          <div className="bank-box">
            <div className="label">Informations bancaires:</div>
            <div className="content">
        ...
        <div className="validity-notice">
          Ce devis est valide jusqu'au {formatDate(props.validUntil)}
        </div>
