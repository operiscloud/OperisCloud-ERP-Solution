...
export async function generateQuotePDF(data: QuoteData): Promise<Uint8Array> {
  ...
  const primaryColor = config.primaryColor ? hexToRgb(config.primaryColor) : rgb(0.06, 0.73, 0.51);
  const secondaryColor = config.secondaryColor ? hexToRgb(config.secondaryColor) : rgb(0.95, 0.95, 0.95);
  const accentColor = config.accentColor ? hexToRgb(config.accentColor) : rgb(0.86, 0.15, 0.15);
  ...
  if (showCompanyInfo) {
    ...
    if (data.companyCountry) {
      page.drawText(data.companyCountry, { x: 50, y: yPos, size: 10, font, color: rgb(0, 0, 0) });
      yPos -= 15;
    }
    ...
    if (data.companyWebsite) {
      page.drawText(`Web: ${data.companyWebsite}`, { x: 50, y: yPos, size: 10, font, color: rgb(0, 0, 0) });
      yPos -= 15;
    }
    if (data.taxNumber) {
  ...
  page.drawText(`N° ${data.quoteNumber}`, {
    x: width - 150,
    y: height - 80,
    size: 10,
    font,
  ...
  page.drawText(`Date: ${formatDate(data.quoteDate)}`, {
    x: width - 150,
    y: height - 95,
    size: 10,
    font,
  ...
  page.drawText(`Valide jusqu'au: ${formatDate(data.validUntil)}`, {
    x: width - 150,
    y: height - 110,
    size: 10,
    font,
  ...
  page.drawText('Devis pour:', { x: 50, y: yPos, size: 12, font: fontBold, color: rgb(0, 0, 0) });
  yPos -= 20;
  ...
  if (data.customerPhone) {
    page.drawText(data.customerPhone, { x: 50, y: yPos, size: 10, font, color: rgb(0, 0, 0) });
    yPos -= 15;
  }
  ...
  page.drawText('Code', { x: 60, y: tableTop - 15, size: 10, font: fontBold, color: rgb(1, 1, 1) });
  page.drawText('Description', { x: 150, y: tableTop - 15, size: 10, font: fontBold, color: rgb(1, 1, 1) });
  ...
  data.items.forEach((item, index) => {
    ...
    page.drawText(item.sku || '-', { x: 60, y: yPos, size: 10, font, color: rgb(0, 0, 0) });
    page.drawText(itemName, { x: 150, y: yPos, size: 10, font, color: rgb(0, 0, 0) });
  ...
  if (data.notes) {
    yPos -= 40;
    page.drawText('Notes:', { x: 50, y: yPos, size: 10, font: fontBold, color: rgb(0, 0, 0) });
    yPos -= 15;
  ...
  // Bank Information - with background box (on quotes too for payment in advance)
  if (showBankInfo && data.bankIBAN) {
    yPos -= (data.notes ? 20 : 40);
  ...
  // Validity notice
  yPos -= (data.notes || (showBankInfo && data.bankIBAN) ? 20 : 40);
  page.drawText(
